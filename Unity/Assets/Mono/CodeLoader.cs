	...
	public class CodeLoader: IDisposable
	{
		...
		public void Start()
		{
			switch (this.CodeMode)
			{
				case CodeMode.Mono:
				{
                        //byte[] assBytes = AssetComponent.Load<TextAsset>(BPath.assets_bundles_code_code_dll_bytes, "Code").bytes;					
                        //byte[] pdbBytes = AssetComponent.Load<TextAsset>(BPath.assets_bundles_code_code_pdb_bytes, "Code").bytes;
					...
					IStaticMethod start = new MonoStaticMethod(assembly, "ET.GSEntry", "Start");						
					start.Run();						
					break;
				}
				case CodeMode.ILRuntime:
				{
					...
					IStaticMethod start = new ILStaticMethod(appDomain, "ET.GSEntry", "Start", 0);
					start.Run();
					break;
				}
